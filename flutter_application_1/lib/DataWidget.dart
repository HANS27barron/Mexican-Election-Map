...
class DataWidget extends StatelessWidget{
  ...
  const DataWidget({super.key, required this.selectedState, required this.results});
  Widget build(BuildContext context){
    return ValueListenableBuilder(valueListenable: selectedState, builder: (context, state, _){
          return ValueListenableBuilder(valueListenable: results, builder: (context, results, _){
      return Container(width: 500, height: 575, child: 
            Column(children: [Container(width: double.infinity, padding: EdgeInsets.all(10), decoration: BoxDecoration(color: Color.fromARGB(255, 177, 22, 11), borderRadius: BorderRadius.circular(9)),
                      child: Text(state, textAlign: TextAlign.center,
                              ...
                              SizedBox(height: 15,),
                              Expanded(child: Container(child: Text(results), height:double.infinity, width: double.infinity, decoration: BoxDecoration(color: Color.fromARGB(255, 177, 22, 11), borderRadius: BorderRadius.circular(9)),))],),);
  }
    );
